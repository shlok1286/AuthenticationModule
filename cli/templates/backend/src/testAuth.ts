...
async function runAuthTests() {
  ...
  try {
    ...
    if (userDoc) {
      ...
      const tokenHash = await bcrypt.hash(plainTestOTP, 10);
      ...
      const correctData: any = await correctRes.json();
  ...
  try {
    ...
    const tokenHash = await bcrypt.hash(resetOTP, 10);
    await OTP.deleteMany({ email: testEmail, purpose: 'reset-password' });
    await OTP.create({
      email: testEmail,
      tokenHash,
